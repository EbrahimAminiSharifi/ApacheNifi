...
@Tags({"sql", "jdbc", "cdc", "mysql", "custom"})
@CapabilityDescription("1.4.34 Retrieves Change Data Capture (CDC) events from a MySQL database. CDC Events include INSERT, UPDATE, DELETE operations. Events "
...
public class CaptureChangeMySQL extends AbstractSessionFactoryProcessor {
    ...
    private Map<Long, TableInfo> mapTableAndId = new HashMap<>();
                            ...
                            if (currentTable == null) {
                                ...
                                try {
                                    ...
                                    try {
                                        ...
                                        mapTableAndId.putIfAbsent(data.getTableId(), currentTable);
                ...
                case QUERY:
                    ...
                    if ("BEGIN".equals(sql)) {
                        ...
                        if (inTransaction) {
                            ...
                            log.error("COMMIT event received while not processing a transaction (i.e. no corresponding BEGIN event). "
...
//                    }
                    ...
                    if (eventType == WRITE_ROWS
                        ...
                        WriteRowsEventData wEvent = (WriteRowsEventData) event.getData();
                        currentTable = mapTableAndId.getOrDefault(wEvent.getTableId(), currentTable);
                        if (currentTable == null) {
                            break;
                        }
                        if (!tableNamePattern.matcher(currentTable.getTableName()).matches()) {
                            break;
                        }
                        ...
                        DeleteRowsEventData wEvent = (DeleteRowsEventData) event.getData();
                        currentTable = mapTableAndId.getOrDefault(wEvent.getTableId(), currentTable);
                        if (currentTable == null) {
                            break;
                        }
                        ...
                        UpdateRowsEventData wEvent = (UpdateRowsEventData) event.getData();
                        currentTable = mapTableAndId.getOrDefault(wEvent.getTableId(), currentTable);
                        if (currentTable == null) {
                            break;
                        }
     ...
     * @param q A queue used to communicate events between the listener and the
     * NiFi processor thread.
     * @return A BinlogEventListener instance, which will be notified of events
     ...
     * The column information can be used to enrich CDC events coming from the
     * RDBMS.
     *
     * @param key A TableInfoCacheKey reference, which contains the database and
     * table names
